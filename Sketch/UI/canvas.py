...
from Sketch import _, InvertingDevice,GraphicsDevice,  HitTestDevice, StandardColors, Point
...
class SketchCanvas(SketchView, CursorStack, WidgetWithModes):
    ...
    document = None
    commands = None
    #unlink all and'SkipAClone', was removed from here
    context_menu_items = ['InterpolateToRoot', 'BreakClones','UnlinkClone',None,'CreateFillStyleDialog',
			  'FillSolid',
			  'FillNone',
			  None,
			  ...
			  None,
			  'FitSelectedToWindow',
			  'FitPageToWindow',None,'CreateTiledClones' ]
    ...
    def SetDocumentDevice(self ):
        self.document.device = self.device
        self.document.graphics_device = self.graphics_device
    def resetgc(self):
        self.device.init_gc(self.tkwin)
        self.invgc.init_gc(self.tkwin)
    def init_gcs(self):
        # XXX: integrate with SketchView
        self.gc.init_gc(self.tkwin, graphics_exposures = 1)    
        self.device = self.invgc =  InvertingDevice() #self.invgc = self.document.device = InvertingDevice()
        self.graphics_device = self.gc
        self.graphics_device.init_gc(self.tkwin)
        
        self.SetDocumentDevice()
        self.invgc.init_gc(self.tkwin)
	...
	# handle button press event    
        #----------------------------------------------------
        #added by shumon June 9, 2009 to handle scroll spacing
        button = event.num
        if button == const.Button4:
           self.document.change_clone_spacing(-1)
           self.update_current_info_text() #to update "clone =" after scrolling
           return        
    	...
    	try:
    	    ...
    	    if button == const.Button1:
        		...
        		self.mode.button_down(p, button, event.state)
    		     ...
    		     self.popup_context_menu(event.x_root, event.y_root, button, event.state)
    		     self.ignore_key_press_events = 0
    ...
    def ButtonReleaseEvent(self, event):
        ...
        #added by shumon June 9, 2009 to handle scroll spacing
        button = event.num
        if button == const.Button5 or button == const.Button4:            
	...
	try:
	    ...
	    sym = event.keysym
            #print sym
            #if sym =='Shift_L':
            #    self.document.is_drag_creating = 1
            char = event.char
            try:
		...
		return
                ...
                #print "z"
                pass #self.document.change_clone_spacing(-1) #-1 decrease
            elif sym == 'x' or sym == 'X':
                #print "x"
                pass#self.document.change_clone_spacing(1) #1 means increase
            elif sym == 'c' or sym == 'C':
                self.document.Hide(self.document.device)
                self.document.ClearDragCreationData()
                self.document.stop_drag = 1
                self.document.Show(self.document.device)
            #elif sym == 'a' or sym == 'A':
             #   print "a"                
             #   self.document.ClearDragCreationData()
             #   self.document.view_redraw_all()
             #   self.document.clones_aligned_permanently = 1
    ...
    #don't do this if drag creation
    #if last action was DragCreation. Create clones here
     self.handle_update_installed = 0
     self.set_handles(self.document.GetSelectionHandles())
   #  if (self.document.LastActionWasDragCreation()):
    #     self.document.DragCreation()
    #     self.document.ClearDragCreationData()
         #return
    ...
    def draw_handles(self):
        #print "inv=", self.device
        self.document.draw_arrows()
    ...
    #
    # old skipping clone interface now integrated into dialog box
    #
    
    #SkipACloneText =  _("Skip a Clone...")
    
   # AddSelCmd('SkipAClone', _("Skip a Clone..."), key_stroke = 'F9',  sensitive_cb = 'CanSkipAClone')
    #def SkipAClone(self):
        #print "setting clone skip = 1..."
        #self.document.skip = 2      
    
    #def CanSkipAClone(self):
        #return self.document.clones != []
    
    #
    #	Fill
    #
    
    AddSelCmd('InterpolateToRoot', _("Interpolate to master"),  sensitive_cb = 'CanInterpolateToRoot')
    AddSelCmd('UnlinkClone', _("Unlink object"),  sensitive_cb = 'CanUnlinkClone')
    #AddSelCmd('UnlinkAll', _("Unlink all Clones in the series..."),  sensitive_cb = 'CanUnlinkAll') #this is irreversable, only possible to undo
    AddSelCmd('BreakClones', _("Break dependency at this object"),  sensitive_cb = 'CanBreakClones')
    AddSelCmd('CreateTiledClones', _("Create Tiled Clones"),  sensitive_cb = 'CanCreateTiledClones')
    
    def CreateTiledClones(self):
        
        self.document.CreateTiledClonesDialog()
        
       
    def CanCreateTiledClones(self):
        
        objs = self.document.selection.GetObjects()
        #don't allow the user to use tiled clones if he is not supposed to (for experiment
        if (self.document != None and self.document.main_window != None):
            technique = self.document.main_window.application.technique
            if technique != _("t") and technique != None:
                #self.main_window.reopen_log_file() 
                return 0
        
        return objs != []
    
    def InterpolateToRoot(self):
        print "interpolating colors to root"
        self.document.InterpolateToRoot()

    def CanInterpolateToRoot(self):
        return self.document.CanInterpolateToRoot()
    
    def UnlinkClone(self):
        print "unlinking clone..."
        self.document.UnlinkClone()
    def CanUnlinkClone(self):
        return self.document.CanUnlinkClone()
   
    def UnlinkAll(self):
        print "unlinking all clones..."
        self.document.UnlinkAll()
    def CanUnlinkAll(self):
        return self.document.CanUnlinkAll() 
    
    def BreakClones(self):
        print "breaking clones at current clone..."
        self.document.BreakClones()
    def CanBreakClones(self):
        return self.document.CanBreakClones()
    ...
    def FillSolid(self, col = None):
        ...
        # interactively select a color.
            #go through the recursive algorithm in EditClones to apply to all other objects if applicable
            self.document.EditClones(self.fill_solid, col)
            #self.fill_solid(col)
 
    def fill_solid(self, col):        
        if col is None:
            import colordlg
